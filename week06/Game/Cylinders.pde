...
boolean checkCylindersBall() {
  PVector mouse = new PVector(mouseX-boxCenterX, boxCenterY-mouseY);
  return (mouse.dist(ball.getLocation()) > radiusBall + cylinderBaseSize);
}

boolean checkCylindersCylinders() {
  PVector mouse = new PVector(mouseX-boxCenterX, boxCenterY-mouseY);
  for (PVector c : cylinders) {
    if (mouse.dist(c) < 2*cylinderBaseSize) return false;
  }
  return true;
}
void addCylinder() {
