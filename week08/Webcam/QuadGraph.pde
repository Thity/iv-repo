...
class QuadGraph {
  ...
  PVector[] quadVertices(List<PVector> allLines, int[] lineIndex) {
    PVector[] lines = new PVector[lineIndex.length];
    for (int i=0; i<lineIndex.length; i++) {
      lines[i] = allLines.get(lineIndex[i]);
    }
    PVector[] vertices = new PVector[lineIndex.length];
    for (int i=0; i<lines.length; i++) {
      vertices[i] = intersection(lines[i], lines[(i+1) % lines.length]);
    }
    return vertices;
  }
  ...
  void findNewCycles(int[] path)
  {
    ...
    for (int i = 0; i < graph.length; i++)
      for (int y = 0; y <= 1; y++)
        ...
        {
          x = graph[i][(y + 1) % 2];
          if (!visited(x, path) && path.length <= 3)
            // Added && path.length <= 3 to make algorithm faster
